...
class FastTextClassificationModel(HybridBlock):
    ...
    def __init__(self, vocab_size, embedding_dim, num_classes, **kwargs):
        ...
        with self.name_scope():
            ...
            if num_classes == 2:
                num_output_units = 1
            log.info('Number of output units in the last layer :%s',
                         num_output_units)
...
def read_input_data(filename):
    ...
    log.info('Opening file %s for reading input', filename)
...
def parse_args():
    ...
    group.add_argument(
        '--log_file', type=str, help='Location to save log')
    group.add_argument(
...
def get_label_mapping(train_labels, test_labels):
    """
    Create the mapping from label to numeric label
    """
    sorted_labels = np.sort(np.unique(train_labels+test_labels))
    label_mapping = {}
    for i, label in enumerate(sorted_labels):
        label_mapping[label] = i
    log.info('Label mapping:%s', format(label_mapping))
...
def convert_to_sequences(dataset, vocab):
    ...
    log.info('Done! Sequence conversion Time={:.2f}s, #Sentences={}'
...
def preprocess_dataset(dataset, labels):
    ...
    log.info('Done! Preprocessing Time={:.2f}s, #Sentences={}'
                 .format(end - start, len(dataset)))
...
def train(args):
    ...
    log.info('Ngrams range for the training run : %s', ngram_range)
    ...
    log.info('Vocabulary size: %s', len(train_vocab))
    log.info('Training data converting to sequences...')
    ...
    if ngram_range >= 2:
        log.info('Adding %s-gram features', ngram_range)
        ...
        log.info('Added n-gram features to train and test datasets!! ')
    log.info('Encoding labels')

    label_mapping = get_label_mapping(train_labels, test_labels)
    ...
    log.info('Number of labels: %s', num_classes)
    log.info('Initializing network')
    ctx = get_context(args)
    log.info('Running Training on ctx:%s', ctx)
    embedding_dim = args.emsize
    log.info('Embedding Matrix Length:%s', embedding_matrix_len)
    ...
    log.info('Loss function for training:%s', loss_function)
    ...
    log.info('Training on %s samples and testing on %s samples',
                 len(train_data), len(test_data))
    log.info('Number of batches for each epoch : %s, Display cadence: %s',
    ...
    for epoch in range(num_epochs):
        for batch, ((data, length), label) in enumerate(train_dataloader):
            ...
            if batch % display_batch_cadence == 0:
                log.info('Epoch : %s, Batches complete :%s', epoch, batch)
        log.info('Epoch complete :%s, Computing Accuracy', epoch)
        test_accuracy, test_loss = evaluate_accuracy(
            test_dataloader, net, ctx, loss_function, num_classes)
        log.info('Epochs completed : %s Test Accuracy: %s, Test Loss: %s',
...
if __name__ == '__main__':
    ...
    log = Logger(arguments.log_file, level='debug')
    log = log.logger
    train(arguments)
